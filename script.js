...
function loadServicesGrid(zone, containerId){
  ...
  if (!grid) return;
  supaRequest("services_golden?zone=eq." + encodeURIComponent(zone) + "&order=created_at.desc")
    .then(function(rows){
      if (!rows || rows.length === 0){
      ...
      grid.innerHTML = rows.map(function(d){
        ...
        return '<a class="service-card" href="service-detail.html?id=' + d.id + '">' +
          (thumb ? '<img src="' + esc(thumb) + '" alt="' + esc(d.name) + '" loading="lazy">' : '<span class="service-card-noimg"></span>') +
