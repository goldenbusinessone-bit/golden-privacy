...
function openPostModal(d){
  ...
  // Compte une vue par publication (une fois par session/appareil)
  const viewKey = "viewed_realisation_" + d.id;
  if (!sessionStorage.getItem(viewKey)){
    sessionStorage.setItem(viewKey, "1");
    supaRequest("rpc/increment_realisation_view", { method: "POST", body: { r_id: d.id } }).catch(function(){});
  }
...
function loadPodcasts(containerId){
  ...
  supaRequest("podcasts_golden?order=created_at.desc").then(function(rows){
    ...
    // Compte une vue par podcast affiché (une fois par session/appareil)
    rows.forEach(function(d){
      const viewKey = "viewed_podcast_" + d.id;
      if (!sessionStorage.getItem(viewKey)){
        sessionStorage.setItem(viewKey, "1");
        supaRequest("rpc/increment_podcast_view", { method: "POST", body: { p_id: d.id } }).catch(function(){});
      }
    });
  }).catch(function(err){
...
function runSiteSearch(query){
  ...
  supaRequest("services_golden?name=ilike." + encodeURIComponent(query) + "*&is_public=eq.true&select=id,name,zone,price,images&order=name.asc&limit=15")
    .then(function(rows){
      if (!rows || rows.length === 0){
