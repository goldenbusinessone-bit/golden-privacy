...
// ============================================================
function pickLang(row, frField, enField, swField){
  const lang = localStorage.getItem("golden_lang") || "fr";
  if (lang === "en" && row[enField]) return row[enField];
  if (lang === "sw" && row[swField]) return row[swField];
  return row[frField] || "";
}
...
// en anglais et swahili avant l'enregistrement en base.
// field1/field2 = ex. "name"/"description" ou "title"/"description"
// Renvoie { [field1+"_en"]:.., [field2+"_en"]:.., [field1+"_sw"]:.., [field2+"_sw"]:.. }
// ============================================================
function autoTranslateFields(field1, value1, field2, value2){
  const jobs = [];
  const keys = [];
  if (value1){
    jobs.push(callGoldenAI("translate", value1, "anglais")); keys.push(field1 + "_en");
    jobs.push(callGoldenAI("translate", value1, "swahili")); keys.push(field1 + "_sw");
  }
  if (value2){
    jobs.push(callGoldenAI("translate", value2, "anglais")); keys.push(field2 + "_en");
    jobs.push(callGoldenAI("translate", value2, "swahili")); keys.push(field2 + "_sw");
  }
  ...
  supaRequest("services_golden?zone=eq." + encodeURIComponent(zone) + "&is_public=eq.true&order=created_at.desc")
    .then(function(rows){
      ...
      grid.innerHTML = rows.map(function(d){
        ...
        const displayName = pickLang(d, "name", "name_en", "name_sw");
        const displayDesc = pickLang(d, "description", "desc_en", "desc_sw");
...
function loadPodcasts(containerId){
  ...
  supaRequest("podcasts_golden?order=created_at.desc").then(function(rows){
    ...
    container.innerHTML = rows.map(function(d){
      ...
      const pTitle = pickLang(d, "title", "title_en", "title_sw");
      const pDesc = pickLang(d, "description", "desc_en", "desc_sw");
      if (d.audio_url){
