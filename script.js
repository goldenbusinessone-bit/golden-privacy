...
document.addEventListener("DOMContentLoaded", initGoldenAds);
...
function buildGoldenChatContext(){
  ...
  return Promise.all([
    ...
    supaRequest("formations_golden?is_public=eq.true&select=title,level,duration,price&order=created_at.desc&limit=15"),
    supaRequest("bibliotheque_templates_golden?is_public=eq.true&select=title,is_premium,price&order=created_at.desc&limit=15")
  ]).then(function(results){
    const services = results[0] || [];
    const produits = results[1] || [];
    const podcasts = results[2] || [];
    const formations = results[3] || [];
    const templates = results[4] || [];
    let ctx = "Voici le contenu et les fonctionnalités actuellement disponibles sur le site Golden — base-toi UNIQUEMENT sur ces informations réelles pour répondre (n'invente jamais un prix ou un article qui n'y figure pas) :\n\n";
    ctx += "Le site propose aussi une 📚 Bibliothèque Golden (bibliotheque.html) : un éditeur graphique en ligne où les clients connectés créent des affiches, logos et designs (texte, formes, images, effets photo), avec une galerie de modèles gratuits et premium. Et une 🎓 Golden Académie (academie.html) : des formations en ligne.\n\n";
    if (services.length){
    ...
    if (formations.length){
      ctx += "Formations Golden Académie :\n" + formations.map(function(f){ return "- " + f.title + (f.level ? " (" + f.level + ")" : "") + (f.price ? " — " + (typeof f.price === "number" ? f.price.toFixed(2) + " $" : f.price) : " — gratuit"); }).join("\n") + "\n\n";
    }
    if (templates.length){
      ctx += "Modèles disponibles dans la Bibliothèque Golden :\n" + templates.map(function(t){ return "- " + t.title + (t.is_premium ? " — premium, " + (t.price ? t.price.toFixed(2) + " $" : "") : " — gratuit"); }).join("\n") + "\n\n";
    }
...
// ---- IP du visiteur — réutilisée pour le journal de visite ET pour
// la traçabilité des commandes/recharges (résolution mise en cache pour
// la durée de la page, pour ne pas relancer l'appel plusieurs fois) ----
let goldenVisitorIpCache = null;
function getVisitorIp(){
  if (goldenVisitorIpCache) return Promise.resolve(goldenVisitorIpCache);
  return fetch("https://ipapi.co/json/")
    .then(function(r){ return r.json(); })
    .then(function(geo){
      goldenVisitorIpCache = geo.ip || null;
      return goldenVisitorIpCache;
    })
    .catch(function(){ return null; }); // ne bloque jamais une commande si l'IP est indisponible
}
...
function generateGoldenAIImage(prompt){
  return fetch(SUPABASE_URL + "/functions/v1/gemini-image-proxy-ts", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "apikey": SUPABASE_ANON_KEY,
...
// Décode un jeton JWT et vérifie s'il est expiré (sans appel réseau).
// Si le jeton est illisible, on le considère prudemment comme expiré.
...
function getSupaToken(){
  const token = localStorage.getItem("golden_admin_token");
  if (token && isJwtExpired(token)){
    // Jeton périmé : on l'efface pour que toutes les pages du site retombent
    // automatiquement sur la clé publique, au lieu d'envoyer un jeton invalide
    // qui ferait échouer TOUTES les requêtes (même pour les visiteurs anonymes).
    localStorage.removeItem("golden_admin_token");
    localStorage.removeItem("golden_admin_refresh_token");
    return null;
  }
  return token;
}
...
// ============================================================
// 📢 PUBLICITÉS INTERSTITIELLES — chantier 3
// Réglages de timing volontairement simples et modifiables ici :
// à ajuster plus tard selon les vrais chiffres de trafic/marché.
// - 1ère pub : 2 minutes après le début de la visite (toutes pages confondues)
// - puis une pub toutes les 10 minutes tant que le visiteur reste sur le site
// - annulable/ignorable après 5 secondes ; le clic sur la pub (à tout moment)
//   redirige vers le lien annoncé (site externe ou Play Store)
// Pas de pub pour l'admin connecté (pas de sens de se pub à soi-même).
// ============================================================
const GOLDEN_ADS_FIRST_DELAY_MS = 30 * 1000;      // 30 secondes — réglage de TEST, à repasser à 2 min plus tard
const GOLDEN_ADS_REPEAT_DELAY_MS = 5 * 60 * 1000; // 5 minutes — réglage de TEST, à repasser à 10 min plus tard
const GOLDEN_ADS_SKIP_AFTER_MS = 5000;             // délai avant que "Ignorer" devienne actif

// 🔧 MODE DEBUG TEMPORAIRE — affiche un petit bandeau à l'écran pour voir
// exactement ce que fait le moteur de pub, sans avoir besoin de la console
// développeur (pratique sur mobile). À retirer une fois le souci résolu :
// il suffit de repasser GOLDEN_ADS_DEBUG à false.
const GOLDEN_ADS_DEBUG = false;
function goldenAdsDebugToast(msg){
  if (!GOLDEN_ADS_DEBUG) return;
  let el = document.getElementById("golden-ads-debug");
  if (!el){
    el = document.createElement("div");
    el.id = "golden-ads-debug";
    el.style.cssText = "position:fixed;bottom:90px;left:10px;right:10px;background:#14120F;color:#fff;font-size:11.5px;line-height:1.4;padding:10px 12px;border-radius:8px;z-index:10000;box-shadow:0 4px 16px rgba(0,0,0,.35);";
    document.body.appendChild(el);
  }
  el.textContent = "📢 Debug pub : " + msg;
  clearTimeout(el._hideTimer);
  el._hideTimer = setTimeout(function(){ if (el.parentNode) el.remove(); }, 12000);
}

function initGoldenAds(){
  // Pas de pub sur le panel admin, ni pour l'admin connecté sur le site public
  if (location.pathname.indexOf("admin-1.html") !== -1) return;
  if (typeof isSupaLoggedIn === "function" && isSupaLoggedIn()){
    goldenAdsDebugToast("désactivé car session admin active dans ce navigateur.");
    return;
  }

  const SESSION_KEY = "golden_ads_session_start";
  const LAST_SHOWN_KEY = "golden_ads_last_shown";

  let sessionStart = parseInt(sessionStorage.getItem(SESSION_KEY) || "0", 10);
  if (!sessionStart){
    sessionStart = Date.now();
    sessionStorage.setItem(SESSION_KEY, String(sessionStart));
  }
  const lastShown = parseInt(sessionStorage.getItem(LAST_SHOWN_KEY) || "0", 10);
  const nextThreshold = lastShown ? (lastShown + GOLDEN_ADS_REPEAT_DELAY_MS) : (sessionStart + GOLDEN_ADS_FIRST_DELAY_MS);
  const delay = Math.max(nextThreshold - Date.now(), 1500);

  goldenAdsDebugToast("moteur chargé, prochaine vérification dans " + Math.round(delay / 1000) + "s.");
  setTimeout(showGoldenAd, delay);
}

function injectGoldenAdsStyle(){
  if (document.getElementById("golden-ads-style")) return;
  const style = document.createElement("style");
  style.id = "golden-ads-style";
  style.textContent =
    '.golden-ad-overlay{position:fixed;inset:0;background:rgba(20,18,15,.92);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px;}' +
    '.golden-ad-card{background:#fff;border-radius:14px;max-width:420px;width:100%;overflow:hidden;position:relative;box-shadow:0 12px 40px rgba(0,0,0,.4);}' +
    '.golden-ad-label{position:absolute;top:10px;left:12px;background:rgba(20,18,15,.65);color:#fff;font-size:10.5px;font-weight:700;letter-spacing:.5px;text-transform:uppercase;padding:3px 9px;border-radius:20px;z-index:2;}' +
    '.golden-ad-skip{position:absolute;top:10px;right:10px;z-index:2;background:rgba(20,18,15,.65);color:#fff;border:none;border-radius:50%;width:30px;height:30px;font-size:14px;cursor:pointer;display:flex;align-items:center;justify-content:center;}' +
    '.golden-ad-skip[disabled]{opacity:.6;cursor:default;}' +
    '.golden-ad-media{width:100%;aspect-ratio:4/3;background:var(--stone-line,#eee);cursor:pointer;}' +
    '.golden-ad-media img,.golden-ad-media video{width:100%;height:100%;object-fit:cover;display:block;}' +
    '.golden-ad-body{padding:16px 18px 20px;text-align:center;}' +
    '.golden-ad-title{font-size:15px;font-weight:700;color:var(--ink,#14120F);margin-bottom:10px;}' +
    '.golden-ad-cta{display:inline-block;background:var(--gold,#C9A15A);color:var(--ink,#14120F);font-size:13.5px;font-weight:700;padding:10px 22px;border-radius:8px;text-decoration:none;cursor:pointer;}';
  document.head.appendChild(style);
}

function showGoldenAd(){
  goldenAdsDebugToast("requête envoyée à ads_golden...");
  supaRequest("ads_golden?is_active=eq.true").then(function(ads){
    if (!ads || ads.length === 0){
      goldenAdsDebugToast("réponse reçue mais 0 publicité active trouvée.");
      return;
    }
    goldenAdsDebugToast(ads.length + " publicité(s) active(s) trouvée(s), affichage...");
    const ad = ads[Math.floor(Math.random() * ads.length)];
    sessionStorage.setItem("golden_ads_last_shown", String(Date.now()));
    renderGoldenAd(ad);
    supaRequest("rpc/increment_ad_view", { method: "POST", body: { a_id: ad.id } }).catch(function(){});
  }).catch(function(err){
    goldenAdsDebugToast("ERREUR : " + err.message);
    console.error("Erreur chargement publicité :", err);
  });
}

function renderGoldenAd(ad){
  injectGoldenAdsStyle();
  closeGoldenAd(); // sécurité, au cas où une pub serait déjà affichée

  const overlay = document.createElement("div");
  overlay.className = "golden-ad-overlay";
  overlay.id = "golden-ad-overlay";

  const mediaHtml = ad.media_url
    ? (ad.media_type === "video"
        ? '<video src="' + esc(ad.media_url) + '" autoplay muted loop playsinline></video>'
        : '<img src="' + esc(ad.media_url) + '" alt="' + esc(ad.title || "Publicité") + '">')
    : '';

  overlay.innerHTML =
    '<div class="golden-ad-card">' +
      '<span class="golden-ad-label">Publicité</span>' +
      '<button class="golden-ad-skip" id="golden-ad-skip" disabled>' + Math.ceil(GOLDEN_ADS_SKIP_AFTER_MS / 1000) + '</button>' +
      '<div class="golden-ad-media" id="golden-ad-media">' + mediaHtml + '</div>' +
      '<div class="golden-ad-body">' +
        '<div class="golden-ad-title">' + esc(ad.title || "") + '</div>' +
        '<span class="golden-ad-cta" id="golden-ad-cta">' + (ad.target_type === "playstore" ? "📲 Voir sur le Play Store" : "🔗 Découvrir") + '</span>' +
      '</div>' +
    '</div>';
  document.body.appendChild(overlay);
  document.body.style.overflow = "hidden";
  ...
  function goToTarget(){
    supaRequest("rpc/increment_ad_click", { method: "POST", body: { a_id: ad.id } }).catch(function(){});
    window.open(ad.target_url, "_blank", "noopener");
    closeGoldenAd();
  }
  document.getElementById("golden-ad-media").addEventListener("click", goToTarget);
  document.getElementById("golden-ad-cta").addEventListener("click", goToTarget);

  const skipBtn = document.getElementById("golden-ad-skip");
  let remaining = Math.ceil(GOLDEN_ADS_SKIP_AFTER_MS / 1000);
  const countdown = setInterval(function(){
    ...
    if (remaining <= 0){
      ...
      skipBtn.textContent = "✕";
      skipBtn.addEventListener("click", closeGoldenAd);
    } else {
      skipBtn.textContent = remaining;
    }
  }, 1000);
}

function closeGoldenAd(){
  const existing = document.getElementById("golden-ad-overlay");
  if (existing) existing.remove();
  document.body.style.overflow = "";
}
