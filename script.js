...
function initGoldenAds(){
  ...
  if (typeof isSupaLoggedIn === "function" && isSupaLoggedIn()) return;
...
function showGoldenAd(){
  supaRequest("ads_golden?is_active=eq.true").then(function(ads){
    if (!ads || ads.length === 0) return; // pas de pub configurée : on retentera à la prochaine page
  ...
  }).catch(function(err){ console.error("Erreur chargement publicité :", err); });
}
