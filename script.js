...
if (firebaseConfig.apiKey !== "VOTRE_API_KEY_ICI") {
  ...
  // Auth initialisé AVANT db.settings() : si le réglage Firestore ci-dessous
  // échoue pour une raison quelconque, l'authentification admin doit quand
  // même fonctionner — les deux ne doivent pas dépendre l'un de l'autre.
  ...
  try {
    ...
    // La détection automatique (experimentalAutoDetectLongPolling) peut se
    // tromper sur les réseaux très instables : on force donc le long-polling
    // en permanence pour plus de fiabilité, quitte à perdre un peu de vitesse
    // sur les bonnes connexions.
    db.settings({ experimentalForceLongPolling: true, merge: true });
  } catch (e) {
    console.error("Erreur de configuration Firestore (settings) :", e);
  }
} else {
