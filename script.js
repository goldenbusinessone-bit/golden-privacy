...
function runSiteSearch(query){
  ...
  supaRequest("services_golden?or=(name.ilike.*" + encodeURIComponent(query) + "*,description.ilike.*" + encodeURIComponent(query) + "*)&select=id,name,zone,price,images&limit=15")
    .then(function(rows){
      const googleUrl = "https://www.google.com/search?q=" + encodeURIComponent(query + " Golden Entreprise");
      const googleLink = '<a class="search-google-link" href="' + googleUrl + '" target="_blank" rel="noopener">🔎 Chercher "' + esc(query) + '" sur Google</a>';

      if (!rows || rows.length === 0){
        results.innerHTML = '<p class="cm-empty">Aucun service ne correspond sur le site.</p>' + googleLink;
        return;
      }
